...
if ( ! function_exists( 'neat_breadcrumbs' ) ) {
    function neat_breadcrumbs( $args = array() ) {
        ...
        $separator = '<span class="separator" style="color:'.esc_attr($color).'"> ' . esc_attr( $args['separator_icon'] ) . ' </span>';
        ...
        $html .= '<span class="item-home"><a class="bread-link bread-home" style="color:'.esc_attr($color).'" href="' . get_home_url() . '" title="' . esc_attr( $args['home_title'] ) . '">' . esc_attr( $args['home_title'] ) . '</a></span>';
        $html .= $separator;
        ...
        if ( is_singular( 'post' ) ) {
            ...
            if ( $post->post_parent ) {
                ...
                foreach ( $parents as $parent ) {
                    $html .= '<span class="item-parent item-parent-' . esc_attr( $parent ) . '"><a class="bread-parent bread-parent-' . esc_attr( $parent ) . '" style="color:'.esc_attr($color).'" href="' . esc_url( get_permalink( $parent ) ) . '" title="' . get_the_title( $parent ) . '">' . get_the_title( $parent ) . '</a></span>';
                    $html .= $separator;
                }
            }
            $html .= '<span class="item-current item-' . $post->ID . '"><span style="color:'.esc_attr($color).'" title="' . get_the_title() . '"> ' . get_the_title() . '</span></span>';
            ...
            $html .= '<span class="item-parent"><a class="bread-parent" style="color:'.esc_attr($color).'" href="' . esc_url( $parent_permalink ) . '" title="' . esc_attr( $parent_title ) . '">' . esc_attr( $parent_title ) . '</a></span>';
            $html .= $separator;
            $html .= '<span class="item-current item-' . $post->ID . '"><span style="color:'.esc_attr($color).'" title="' . get_the_title() . '"> ' . get_the_title() . '</span></span>';
        } elseif ( is_singular() ) {

            // custom post type or single page
